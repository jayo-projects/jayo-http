...
import jayo.http.brotli.BrotliDecompressionAlgorithm.Brotli
...
class BrotliInterceptorTest {
    val brotliInterceptor = CompressionInterceptor.of(Brotli, Gzip)
    ...
    fun testNoUncompress() {
        val response = response("https://httpbin.org/brotli", "XXXX".encodeToByteString())
    ...
    fun testFailsUncompress() {
        ...
        assertFailsWith<JayoException> {
            ...
            assertThat(ioe.cause?.cause?.javaClass?.simpleName).isEqualTo("BrotliRuntimeException")
        }
    }
    ...
    fun testSkipUncompressNoContentResponse() {
        val response =
            response("https://httpbin.org/brotli", ByteString.EMPTY) {
                header("Content-Encoding", "br")
                statusCode(204)
