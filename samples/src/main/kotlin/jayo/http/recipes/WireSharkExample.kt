...
import jayo.http.*
import jayo.http.recipes.WireSharkListenerFactory.WireSharkKeyLoggerListener.Launch
import jayo.http.recipes.WireSharkListenerFactory.WireSharkKeyLoggerListener.Launch.CommandLine
import jayo.http.recipes.WireSharkListenerFactory.WireSharkKeyLoggerListener.Launch.Gui
import jayo.tls.ClientTlsSocket
import jayo.tls.Handshake
import jayo.tls.TlsVersion
import jayo.tls.TlsVersion.TLS_1_2
...
/**
 * Logs SSL keys to a log file, allowing Wireshark to decode traffic and be examined with http2 filter. The approach is
 * to hook into JSSE log events for the messages between client and server during handshake and then take the agreed
 * masterSecret from private fields of the session.
 *
 * Prerequisites:
 *  * Execute this class with this VM options `--add-opens java.base/sun.security.ssl=ALL-UNNAMED` because it requires
 *  reflection on Java's internals.
 *  * Install WireShark https://tshark.dev/setup/install/
 *  * On Linux you may also need to execute `sudo chmod +x /usr/bin/dumpcap`
 *
 * Copy WireSharkKeyLoggerListener to your test code to use in development.
 *
 * This logs TLSv1.2 on a JVM (OpenJDK 11+) without any additional code.
 *
 * For TLSv1.3 an existing external tool is required.
 *
...
class WireSharkListenerFactory(
    ...
    class WireSharkKeyLoggerListener(
        ...
        override fun connectionAcquired(
            ...
            if (random != null) {
                val sslSocket = connection.socket() as ClientTlsSocket
...
class WiresharkExample(
    ...
    fun run() {
        ...
        val fbRequest = ClientRequest.builder()
            .url("https://graph.facebook.com/robots.txt?s=fb")
            .get()
        val twitterRequest = ClientRequest.builder()
            .url("https://api.twitter.com/robots.txt?s=tw")
            .get()
        val googleRequest = ClientRequest.builder()
    ...
    private fun sendTestRequest(request: ClientRequest) {
        try {
                ...
                .use {
                    val firstLine =
                        it.body
                            ...
                            .first()
                    if (this.launch != CommandLine) {
                        println("${it.statusCode} ${it.request.url.host} $firstLine")
                    }
                }
...
fun main() {
    // Call this before anything else initializes the JSSE stack.
