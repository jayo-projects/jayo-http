...
* do **publish** task
* go to *build/repos/releases* on **brotli**, **core**, **coroutines**, and **logging-interceptor**
* remove the "maven-metadata.xml" (and all files in the same directory) for each project
* zip the **dev** dir with all projects aggregated. Save it with a name like *jayo-http-0.1.0-alpha1.zip*.
* upload manually on https://central.sonatype.com/publishing (jayo account)
  * do **Publish Component**
  * Deployment name → use release name *jayo-http-X.Y.Z*, use the same name as the zip.
