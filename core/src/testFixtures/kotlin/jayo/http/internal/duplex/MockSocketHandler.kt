...
package jayo.http.internal.duplex
 ...
 *
 * Note from pull-vert: this class is copied as-is from OkHttp's MockWebServer and was not modified.
 */
class MockSocketHandler : SocketHandler {
    ...
    fun receiveRequest(expected: String) =
        apply {
            actions += { stream ->
                ...
                if (actual != expected) throw kotlin.AssertionError("$actual != $expected")
            }
        }
    ...
    fun exhaustRequest() =
        apply {
            actions += { stream ->
                if (!stream.source.exhausted()) throw kotlin.AssertionError("expected exhausted")
            }
        }
    ...
    fun awaitSuccess() {
        val futureTask =
            results.poll(5, TimeUnit.SECONDS)
                ?: throw kotlin.AssertionError("no onRequest call received")
