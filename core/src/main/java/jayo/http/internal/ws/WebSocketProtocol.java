...
public final class WebSocketProtocol {
     ...
     * <p>
     * If this flag is set, the next four bytes represent the mask key. These bytes appear after any additional bytes
     * specified by {@link #B1_MASK_LENGTH}.
     */
     ...
     * <p>
     * If this value is {@link #PAYLOAD_SHORT}, the next two bytes represent the length. If this value is
     * {@link #PAYLOAD_LONG}, the next eight bytes represent the length.
     */
    ...
    static final byte OPCODE_CONTROL_CLOSE = 0x8;
    static final byte OPCODE_CONTROL_PING = 0x9;
    static final byte OPCODE_CONTROL_PONG = 0xa;

    /**
     * Maximum length of frame payload. Larger payloads, if supported by the frame type, can use the special values
     * {@link #PAYLOAD_SHORT} or {@link #PAYLOAD_LONG}.
     */
    static final long PAYLOAD_BYTE_MAX = 125L;

    /**
     * Maximum length of a close message in bytes.
     */
     ...
     */
    static final short CLOSE_CLIENT_GOING_AWAY = 1001;
    ...
    static void toggleMask(final Buffer.@NonNull UnsafeCursor cursor, final byte @NonNull [] key) {
        assert cursor != null;
        assert key != null;
        ...
        do {
            final var buffer = cursor.data;
            var i = cursor.pos;
            final var limit = cursor.limit;
            if (buffer != null) {
                while (i < limit) {
    ...
    static @Nullable String closeCodeExceptionMessage(final int code) {
        if (code < 1000 || code >= 5000) {
            return "Code must be in range [1000,5000): " + code;
        } else if ((code >= 1004 && code <= 1006) || (code >= 1015 && code <= 2999)) {
    ...
    static void validateCloseCode(final int code) {
        final var message = closeCodeExceptionMessage(code);
        if (message != null) {
            throw new IllegalArgumentException(message);
        }
    }

    public static String acceptHeader(final @NonNull String key) {
        assert key != null;
        return ByteString.encode(key + ACCEPT_MAGIC).hash(JdkDigest.SHA_1).base64();
    }
}
