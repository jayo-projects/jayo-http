...
import jayo.http.internal.TestUtils.fragmentBuffer
...
internal class MessageDeflaterInflaterTest {
    ...
    fun `inflate golden value`() {
        ...
        assertThat(inflater.inflate(message)).isEqualTo("Hello inflation!".encodeToByteString())
    }
    ...
    fun `deflate golden value`() {
        ...
        val deflated = deflater.deflate("Hello deflate!".encodeToByteString())
    ...
    fun `inflate deflate`() {
        ...
        val goldenValue = "Hello deflate!".repeat(100).encodeToByteString()

        val deflated = deflater.deflate(goldenValue)
        assertThat(deflated.byteSize()).isLessThan(goldenValue.byteSize())
    ...
    fun `inflate deflate empty message`() {
        ...
        val goldenValue = "".encodeToByteString()
    ...
    fun `deflate after close`() {
        ...
        assertFailsWith<Exception> {
            deflater.deflate("Hello deflate!".encodeToByteString())
        }
    }
    ...
    fun `inflate golden value in buffer that has been fragmented`() {
        ...
        assertThat(buffer.readString()).isEqualTo("Hello inflation!")
    }
    ...
    fun `deflated data has too many bytes`() {
        ...
        val message1 = "hello".encodeToByteString()
        val message2 = "hello 2".encodeToByteString()

        Jayo.deflate(buffer, Deflater(Deflater.DEFAULT_COMPRESSION, true)).use { writer ->
            writer.writeFrom(Buffer().write(message1), message1.byteSize().toLong())
        }
        ...
        Jayo.deflate(buffer, Deflater(Deflater.DEFAULT_COMPRESSION, true)).use { sink ->
            sink.writeFrom(Buffer().write(message2), message2.byteSize().toLong())
        }
        buffer.writeByte(0x00)
        inflater.inflate(buffer)
