...
class Http2ConnectionTest {
    ...
    @Test
    fun peerHttp2ServerLowersInitialWindowSize() {
    ...
    @Test
    fun peerHttp2ServerZerosCompressionTable() {
    ...
    @Test
    fun peerHttp2ClientDisablesPush() {
        ...
        // verify the peer's settings were read and applied.
        assertThat(connection.peerSettings.enablePush(true)).isFalse()
    }

    @Test
    fun peerIncreasesMaxFrameSize() {
        ...
        // verify the peer's settings were read and applied.
        assertThat(connection.peerSettings.maxFrameSize(-1)).isEqualTo(newMaxFrameSize)
    ...
    fun peerSetsZeroFlowControl() {
        ...
        val sink = stream.writer.buffered()
    ...
    fun discardedDataFramesAreCounted() {
        ...
        val source = stream1.reader
        val buffer = Buffer()
        while (buffer.bytesAvailable() != 1024L) {
            source.readAtMostTo(buffer, 1024)
        }
    ...
    fun receiveGoAwayHttp2() {
        ...
        val sink1 = stream1.writer.buffered()
        val sink2 = stream2.writer.buffered()
        sink1.write("abc")
        ...
        sink1.write("def")
        sink1.close()
        assertFailsWith<JayoConnectionShutdownException> {
    ...
    @Test
    fun readSendsWindowUpdateHttp2() {
        val windowSize = 100
        ...
        // Play it back.
        val connection = connect(peer)
        connection.jayoHttpSettings[Settings.INITIAL_WINDOW_SIZE] = windowSize
        ...
        assertThat(stream.takeHeaders(false)).isEqualTo(Headers.of("a", "android"))
        val source = stream.reader
        val buffer = Buffer()
        buffer.writeAllFrom(source)
        assertThat(source.readAtMostTo(buffer, 1)).isEqualTo(-1)
        assertThat(buffer.bytesAvailable()).isEqualTo(150)
        val synStream = peer.takeFrame()
        assertThat(synStream.type).isEqualTo(Http2.TYPE_HEADERS)
        for (i in 0..2) {
            val windowUpdateStreamIds: MutableList<Int?> = kotlin.collections.ArrayList(2)
            for (j in 0..1) {
    ...
    @Test
    fun serverSendsEmptyDataClientDoesntSendWindowUpdateHttp2() {
        // Write the mocking script.
        peer.sendFrame().settings(Settings())
        peer.acceptFrame() // ACK
        peer.acceptFrame() // SYN_STREAM
        ...
        assertThat(client.reader.readAtMostTo(Buffer(), 1)).isEqualTo(-1)
    ...
    @Test
    fun clientSendsEmptyDataServerDoesntSendWindowUpdateHttp2() {
        // Write the mocking script.
        peer.sendFrame().settings(Settings())
        peer.acceptFrame() // ACK
        peer.acceptFrame() // SYN_STREAM
        ...
        val out = client.writer.buffered()
    ...
    fun maxFrameSizeHonored() {
        ...
        val out = stream.writer.buffered()
        out.write(buff)
        out.flush()
        out.close()
        val synStream = peer.takeFrame()
    ...
    fun pushPromiseStream() {
        ...
        val expectedRequestHeaders =
            listOf(
                ...
                RealBinaryHeader(RealBinaryHeader.TARGET_PATH, "/cached"),
            )
        peer.sendFrame().pushPromise(3, 2, expectedRequestHeaders)
        val expectedResponseHeaders =
            listOf(
                RealBinaryHeader(RealBinaryHeader.RESPONSE_STATUS, "200"),
            )
     ...
     */
    @Test
    fun socketExceptionWhileWritingHeaders() {
        peer.acceptFrame() // SYN_STREAM.
        peer.play()
    ...
    @Test
    fun clientCreatesStreamAndServerReplies() {
        // Write the mocking script.
        peer.sendFrame().settings(Settings())
        peer.acceptFrame() // ACK
        peer.acceptFrame() // SYN_STREAM
        peer.acceptFrame() // DATA
        peer.sendFrame().headers(false, 3, headerEntries("a", "android"))
        peer.sendFrame().data(true, 3, Buffer().write("robot"), 5)
        peer.acceptFrame() // PING
        ...
        out.close()
        assertThat(stream.takeHeaders(false)).isEqualTo(Headers.of("a", "android"))
        assertStreamData("robot", stream.reader)
    ...
    @Test
    fun serverFinishesStreamWithHeaders() {
        // Write the mocking script.
        peer.sendFrame().settings(Settings())
        peer.acceptFrame() // ACK
        peer.acceptFrame() // SYN_STREAM
        ...
        connection.writePingAndAwaitPong()
        assertThat(stream.takeHeaders(false)).isEqualTo(Headers.of("headers", "bam"))
    ...
    @Test
    fun serverWritesTrailersAndClientReadsTrailers() {
        // Write the mocking script.
        peer.sendFrame().settings(Settings())
        peer.acceptFrame() // ACK
        peer.acceptFrame() // SYN_STREAM
        ...
        assertThat(stream.takeHeaders(false)).isEqualTo(Headers.of("headers", "bam"))
        connection.writePingAndAwaitPong()
        assertThat(stream.peekTrailers()).isEqualTo(Headers.of("trailers", "boom"))
        assertThat(connection.openStreamCount()).isEqualTo(0)
    ...
    @Test
    fun serverResponseBodyRstStream() {
        // Write the mocking script.
        peer.sendFrame().settings(Settings())
        peer.acceptFrame() // ACK
        peer.acceptFrame() // SYN_STREAM
        ...
        connection.writePingAndAwaitPong()
        assertThat(stream.takeHeaders(false)).isEqualTo(Headers.of("a", "android"))
        val source = stream.reader.buffered()
        assertThat(source.readString(5)).isEqualTo("robot")
        stream.writer.close()
        assertThat(connection.openStreamCount()).isEqualTo(0)
    ...
    fun serverTrailersRstStream() {
        ...
        assertThat(stream.takeHeaders(false)).isEqualTo(Headers.of("a", "android"))
        stream.writer.close()
        assertThat(stream.peekTrailers()).isEqualTo(Headers.of("z", "zebra"))
        assertThat(connection.openStreamCount()).isEqualTo(0)
     ...
     */
    @Test
    fun clientRequestBodyServerResponseBodyRstStream() {
        // Write the mocking script.
        peer.sendFrame().settings(Settings())
        peer.acceptFrame() // ACK
        peer.acceptFrame() // SYN_STREAM
    ...
    @Test
    fun serverWritesTrailersWithData() {
    ...
    @Test
    fun clientCannotReadTrailersWithoutExhaustingStream() {
        // Write the mocking script.
        peer.sendFrame().settings(Settings())
        peer.acceptFrame() // ACK
        peer.acceptFrame() // SYN_STREAM
        peer.sendFrame().data(false, 3, Buffer().write("robot"), 5)
        peer.sendFrame().headers(true, 3, headerEntries("trailers", "boom"))
        peer.acceptFrame() // PING
        peer.sendFrame().ping(true, Http2Connection.AWAIT_PING, 0) // PONG
        peer.play()
    ...
    @Test
    fun clientCannotReadTrailersIfTheStreamFailed() {
        // Write the mocking script.
        peer.sendFrame().settings(Settings())
        peer.acceptFrame() // ACK
        peer.acceptFrame() // SYN_STREAM
    ...
    @Test
    fun serverCannotEnqueueTrailersAfterFinishingTheStream() {
        peer.setClient(true)
        peer.sendFrame().settings(Settings())
        peer.acceptFrame() // ACK
        peer.acceptFrame() // PING
    ...
    @Test
    fun noTrailersFrameYieldsEmptyTrailers() {
        // Write the mocking script.
        peer.sendFrame().settings(Settings())
        peer.acceptFrame() // ACK
        peer.acceptFrame() // SYN_STREAM
    ...
    @Test
    fun serverReadsHeadersDataHeaders() {
        // Write the mocking script.
        peer.sendFrame().settings(Settings())
        peer.acceptFrame() // ACK
        peer.acceptFrame() // SYN_STREAM
    ...
    @Test
    fun clientCreatesStreamAndServerRepliesWithFin() {
        // Write the mocking script.
        peer.sendFrame().settings(Settings())
        peer.acceptFrame() // ACK
        peer.acceptFrame() // SYN_STREAM
    ...
    @Test
    fun unexpectedPongIsNotReturned() {
        // Write the mocking script.
        peer.sendFrame().settings(Settings())
        peer.acceptFrame() // ACK
        peer.sendFrame().ping(false, 2, 0)
    ...
    @Test
    fun serverSendsSettingsToClient() {
        // Write the mocking script.
        val settings = Settings()
        ...
        val listener: Http2Connection.Listener =
            object : Http2Connection.Listener() {
                override fun onStream(stream: Http2Stream): Unit = throw kotlin.AssertionError()
        ...
        connection.lock.withLock {
            assertThat(connection.peerSettings.maxConcurrentStreams()).isEqualTo(10)
        }
    ...
    @Test
    fun multipleSettingsFramesAreMerged() {
        // Write the mocking script.
        ...
        connection.lock.withLock {
            ...
            assertThat(connection.peerSettings.maxFrameSize(-1)).isEqualTo(50000)
            assertThat(connection.peerSettings.maxConcurrentStreams()).isEqualTo(60000)
        }
    }

    @Test
    fun clearSettingsBeforeMerge() {
        // Write the mocking script.
        val settings1 = Settings()
        settings1[Settings.HEADER_TABLE_SIZE] = 10000
        settings1[Settings.INITIAL_WINDOW_SIZE] = 20000
        ...
        connection.lock.withLock {
            ...
            assertThat(connection.peerSettings.maxFrameSize(-1)).isEqualTo(-1)
            assertThat(connection.peerSettings.maxConcurrentStreams()).isEqualTo(60000)
        }
    }

    @Test
    fun bogusDataFrameDoesNotDisruptConnection() {
        // Write the mocking script.
        peer.sendFrame().settings(Settings())
        peer.acceptFrame() // ACK
        peer.sendFrame().data(true, 41, Buffer().write("bogus"), 5)
        peer.acceptFrame() // RST_STREAM
        peer.sendFrame().ping(false, 2, 0)
        peer.acceptFrame() // PING
        peer.play()
    ...
    @Test
    fun serverClosesClientOutputStream() {
        // Write the mocking script.
        peer.sendFrame().settings(Settings())
        peer.acceptFrame() // ACK
        peer.acceptFrame() // SYN_STREAM
     ...
     */
    @Test
    fun clientClosesClientInputStream() {
        // Write the mocking script.
        peer.sendFrame().settings(Settings())
        peer.acceptFrame() // ACK
        peer.acceptFrame() // SYN_STREAM
     ...
     */
    @Test
    fun clientClosesClientInputStreamIfOutputStreamIsClosed() {
        // Write the mocking script.
        peer.sendFrame().settings(Settings())
        peer.acceptFrame() // ACK
        peer.acceptFrame() // SYN_STREAM
    ...
    @Test
    fun serverClosesClientInputStream() {
        // Write the mocking script.
        peer.sendFrame().settings(Settings())
        peer.acceptFrame() // ACK
        peer.acceptFrame() // SYN_STREAM
        peer.sendFrame().headers(false, 3, headerEntries("b", "banana"))
        peer.sendFrame().data(true, 3, Buffer().write("square"), 6)
        peer.acceptFrame() // PING
        peer.sendFrame().ping(true, Http2Connection.AWAIT_PING, 0)
        peer.play()
    ...
    @Test
    fun remoteSendsDataAfterInFinished() {
        // Write the mocking script.
        peer.sendFrame().settings(Settings())
        peer.acceptFrame() // ACK
        peer.acceptFrame() // SYN_STREAM
        peer.sendFrame().headers(false, 3, headerEntries("a", "android"))
        peer.sendFrame().data(true, 3, Buffer().write("robot"), 5)
        peer.sendFrame().data(true, 3, Buffer().write("c3po"), 4)
        peer.acceptFrame() // RST_STREAM
    ...
    @Test
    fun clientDoesNotLimitFlowControl() {
        ...
        val connection = connect(peer)
        val stream = connection.newStream(headerEntries("a", "android"), false)
        assertThat(stream.takeHeaders(false)).isEqualTo(Headers.of("b", "banana"))
    ...
    @Test
    fun remoteSendsRefusedStreamBeforeReplyHeaders() {
        // Write the mocking script.
        peer.sendFrame().settings(Settings())
        peer.acceptFrame() // ACK
        peer.acceptFrame() // SYN_STREAM
    ...
    fun sendGoAway() {
        ...
        connection.lock.withLock {
            if (!connection.isHealthy(System.nanoTime())) {
                throw JayoConnectionShutdownException()
            }
        }
    ...
    fun close() {
        ...
        assertFailsWith<JayoException> {
            stream.reader.readAtMostTo(Buffer(), 1)
        }.also { expected ->
            assertThat(expected.message).isEqualTo("stream was reset: CANCEL")
        }
    ...
    fun getResponseHeadersTimesOut() {
        ...
        val connection = connect(peer)
        val stream = connection.newStream(headerEntries("b", "banana"), false)
        stream.readTimeoutNanos = 500.milliseconds.inWholeNanoseconds
    ...
    /**
     * Confirm that the client times out if the server stalls after 3 bytes. After the timeout the connection is still
    ...
    fun readTimesOut() {
        ...
        peer.acceptFrame() // SYN_STREAM
        peer.sendFrame().headers(false, 3, headerEntries("a", "android"))
        peer.sendFrame().data(false, 3, Buffer().write("abc"), 3)
        peer.acceptFrame() // RST_STREAM
    ...
    fun writeTimesOutAwaitingStreamWindow() {
        ...
        val stream = connection.newStream(headerEntries("b", "banana"), true)
        val sink = stream.writer
        sink.writeFrom(Buffer().write("abcde"), 5)
        stream.writeTimeoutNanos = 500.milliseconds.inWholeNanoseconds
        val startNanos = System.nanoTime()
        sink.writeFrom(Buffer().write("f"), 1)
        assertFailsWith<JayoTimeoutException> {
    ...
    fun writeTimesOutAwaitingConnectionWindow() {
        ...
        val sink = stream.writer
        stream.writeTimeoutNanos = 500.milliseconds.inWholeNanoseconds
        sink.writeFrom(Buffer().write("abcdef"), 6)
        val startNanos = System.nanoTime()
        assertFailsWith<JayoTimeoutException> {
            sink.flush() // This will time out waiting on the write window.
        }
    ...
    fun outgoingWritesAreBatched() {
        ...
        val sink = stream.writer
        sink.writeFrom(Buffer().write("abcde"), 5)
        sink.writeFrom(Buffer().write("fghij"), 5)
        sink.close()
    ...
    fun headers() {
        ...
        peer
            .sendFrame()
            .headers(false, 3, headerEntries(RealBinaryHeader.RESPONSE_STATUS_UTF8, "HTTP/1.1 100"))
        peer
            .sendFrame()
            .headers(false, 3, headerEntries(RealBinaryHeader.RESPONSE_STATUS_UTF8, "HTTP/1.1 200"))
        peer.sendFrame().ping(true, Http2Connection.AWAIT_PING, 0)
        peer.play()
        ...
        assertThat(stream.takeHeaders(false))
            .isEqualTo(Headers.of(RealBinaryHeader.RESPONSE_STATUS_UTF8, "HTTP/1.1 100"))
        assertThat(stream.takeHeaders(false))
            .isEqualTo(Headers.of(RealBinaryHeader.RESPONSE_STATUS_UTF8, "HTTP/1.1 200"))
    ...
    @Test
    fun readMultipleSetsOfResponseHeaders() {
        // Write the mocking script.
        peer.sendFrame().settings(Settings())
        peer.acceptFrame() // ACK
        peer.acceptFrame() // SYN_STREAM
        ...
        assertThat(stream.takeHeaders(false)).isEqualTo(Headers.of("a", "android"))
        connection.writePingAndAwaitPong()
        assertThat(stream.peekTrailers()).isEqualTo(Headers.of("c", "cola"))
    ...
    @Test
    fun serverSendsEmptyDataClientDoesntSendWindowUpdate() {
        // Write the mocking script.
        peer.sendFrame().settings(Settings())
        peer.acceptFrame() // ACK
        peer.acceptFrame() // SYN_STREAM
    ...
    @Test
    fun clientSendsEmptyDataServerDoesntSendWindowUpdate() {
        // Write the mocking script.
        peer.sendFrame().settings(Settings())
        peer.acceptFrame() // ACK
        peer.acceptFrame() // SYN_STREAM
    ...
    fun testTruncatedDataFrame() {
        ...
        assertFailsWith<JayoEOFException> {
            source.buffered().readByteString(101)
        }
    }

    @Test
    fun blockedStreamDoesntStarveNewStream() {
        ...
        val out1 = stream1.writer.buffered()
        ...
        val stream2 = connection.newStream(headerEntries("b", "banana"), true)
        val out2 = stream2.writer.buffered()
        out2.write("foo")
        out2.flush()
        assertThat(connection.writeBytesTotal)
            .isEqualTo((Settings.DEFAULT_INITIAL_WINDOW_SIZE + 3).toLong())
    ...
    @Test
    fun remoteOmitsInitialSettings() {
    ...
    private fun assertStreamData(
        ...
        val actual = source!!.buffered().readString()
        assertThat(actual).isEqualTo(expected)
    }

    /**
     * Returns true when all works currently in progress by the watchdog have completed. This method creates more work
    ...
    private class RecordingPushObserver : PushObserver {
        ...
        override fun <T : BinaryHeader?> onRequest(
            streamId: Int,
            requestHeaders: List<T & Any>
        ): Boolean {
            lock.withLock {
        ...
        override fun <T : BinaryHeader?> onHeaders(
            streamId: Int,
            responseHeaders: List<T & Any>,
            last: Boolean
        ): Boolean {
            lock.withLock {
                assertThat(streamId).isEqualTo(2)
        ...
        override fun onData(
            ...
            lock.withLock {
                events.add(kotlin.AssertionError("onData"))
                condition.signalAll()
                return false
            }
        }
        ...
        override fun onReset(
            ...
            lock.withLock {
                events.add(kotlin.AssertionError("onReset"))
                condition.signalAll()
            }
        }
    }
