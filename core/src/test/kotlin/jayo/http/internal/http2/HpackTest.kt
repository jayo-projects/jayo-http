...
class HpackTest {
    ...
    /**
     * Variable-length quantity special cases strings which are longer than 127 bytes.  Values such as cookies can be
     * 4KiB, and should be possible to send.
     *
    ...
    fun tooLargeToHPackIsStillEmitted() {
        ...
        bytesIn.write("custom-key")
        bytesIn.writeByte(0x0d) // Literal value (len = 13)
        bytesIn.write("custom-header")
        hpackReader!!.readHeaders()
        assertThat(hpackReader!!.headerCount).isEqualTo(0)
    ...
    fun writerEviction() {
        ...
        bytesIn.writeByte(0x0a) // Literal name (len = 10)
        bytesIn.write("custom-foo")
        bytesIn.writeByte(0x0d) // Literal value (len = 13)
        bytesIn.write("custom-header")
        bytesIn.writeByte(0x40) // Literal indexed
        bytesIn.writeByte(0x0a) // Literal name (len = 10)
        bytesIn.write("custom-bar")
        bytesIn.writeByte(0x0d) // Literal value (len = 13)
        bytesIn.write("custom-header")
        bytesIn.writeByte(0x40) // Literal indexed
        bytesIn.writeByte(0x0a) // Literal name (len = 10)
        bytesIn.write("custom-baz")
        bytesIn.writeByte(0x0d) // Literal value (len = 13)
        bytesIn.write("custom-header")

        // Set to only support 110 bytes (enough for 2 headers).
        // Use a new Writer because we don't support change the dynamic table size after Writer constructed.
        val writer = Hpack.Writer(110, false, bytesOut)
        writer.writeHeaders(headerBlock)
        assertTrue(bytesOut.contentEquals(bytesIn))
    ...
    fun huffmanDecodingSupported() {
        ...
        bytesIn.writeByte(0x8c.toByte()) // Literal value Huffman encoded 12 bytes
    ...
    fun theSameHeaderAfterOneIncrementalIndexed() {
        ...
        bytesIn.writeByte(0x0d) // Literal value (len = 13)
        bytesIn.write("custom-header")
        bytesIn.writeByte(0xbe.toByte()) // Indexed name and value (idx = 63)
        hpackWriter!!.writeHeaders(headerBlock)
        assertTrue(bytesOut.contentEquals(bytesIn))
        assertThat(hpackWriter!!.headerCount).isEqualTo(1)
        val entry = hpackWriter!!.dynamicTable[hpackWriter!!.dynamicTable.size - 1]!!
    ...
    fun staticHeaderIsNotCopiedIntoTheIndexedTable() {
        bytesIn.writeByte(0x82.toByte()) // == Indexed - Add ==
        // idx = 2 -> :method: GET
        hpackReader!!.readHeaders()
        assertThat(hpackReader!!.headerCount).isEqualTo(0)
    ...
    fun readIndexedHeaderFieldIndex0() {
        bytesIn.writeByte(0x80.toByte()) // == Indexed - Add idx = 0
    ...
    fun readIndexedHeaderFieldTooLargeIndex() {
        bytesIn.writeShort(0xff00.toShort()) // == Indexed - Add idx = 127
        assertFailsWith<JayoException> {
            hpackReader!!.readHeaders()
        }.also { expected ->
    ...
    fun readIndexedHeaderFieldInsidiousIndex() {
        bytesIn.writeByte(0xff.toByte()) // == Indexed - Add ==
    ...
    fun readLiteralHeaderWithIncrementalIndexingStaticName() {
        ...
        assertThat(hpackReader!!.getAndResetHeaderList())
            .containsExactly(RealBinaryHeader("www-authenticate", "Basic"))
    }
    ...
    fun readLiteralHeaderWithIncrementalIndexingDynamicName() {
        ...
        bytesIn.write("Basic2")
        hpackReader!!.readHeaders()
        assertThat(hpackReader!!.getAndResetHeaderList()).containsExactly(
            RealBinaryHeader("custom-foo", "Basic"),
            RealBinaryHeader("custom-foo", "Basic2"),
        )
    }
    ...
    fun readFailingRequestExample() {
        bytesIn.writeByte(0x82.toByte()) // == Indexed - Add ==
        // idx = 2 -> :method: GET
        bytesIn.writeByte(0x86.toByte()) // == Indexed - Add ==
        // idx = 7 -> :scheme: http
        bytesIn.writeByte(0x84.toByte()) // == Indexed - Add ==
        bytesIn.writeByte(0x7f) // == Bad index! ==

        // Indexed name (idx = 4) -> :authority
        bytesIn.writeByte(0x0f) // Literal value (len = 15)
        bytesIn.write("www.example.com")
        assertFailsWith<JayoException> {
            hpackReader!!.readHeaders()
        }.also { expected ->
    ...
    private fun secondRequestWithoutHuffman() {
        ...
        bytesIn.writeByte(0x84.toByte()) // == Indexed - Add ==
        // idx = 6 -> :path: /
        bytesIn.writeByte(0xbe.toByte()) // == Indexed - Add ==
    ...
    private fun thirdRequestWithoutHuffman() {
        bytesIn.writeByte(0x82.toByte()) // == Indexed - Add ==
        // idx = 2 -> :method: GET
        bytesIn.writeByte(0x87.toByte()) // == Indexed - Add ==
        // idx = 7 -> :scheme: http
        bytesIn.writeByte(0x85.toByte()) // == Indexed - Add ==
        // idx = 5 -> :path: /index.html
        bytesIn.writeByte(0xbf.toByte()) // == Indexed - Add ==
        // Indexed name (idx = 63) -> :authority: www.example.com
        bytesIn.writeByte(0x40) // Literal indexed
        bytesIn.writeByte(0x0a) // Literal name (len = 10)
        bytesIn.write("custom-key")
        bytesIn.writeByte(0x0c) // Literal value (len = 12)
        bytesIn.write("custom-value")
    }
    ...
    private fun secondRequestWithHuffman() {
        ...
        bytesIn.writeByte(0x58) // == Literal indexed ==
        // Indexed name (idx = 24) -> cache-control
        bytesIn.writeByte(0x86.toByte()) // Literal value Huffman encoded 6 bytes
    ...
    private fun thirdRequestWithHuffman() {
        ...
        bytesIn.writeByte(0x40) // Literal indexed
        bytesIn.writeByte(0x88.toByte()) // Literal name Huffman encoded 8 bytes
        // decodes to custom-key which is length 10
        bytesIn.write("25a849e95ba97d7f".decodeHex())
        bytesIn.writeByte(0x89.toByte()) // Literal value Huffman encoded 9 bytes
    ...
    fun overflowingValueRejected() {
        assertFailsWith<JayoException> {
            newReader(Buffer().write("8080808008".decodeHex()))
                .readInt(0xff, 0x7f)
        }.also { expected ->
            assertThat(expected.message).isEqualTo("HPACK integer overflow")
        }
    ...
    fun tooManyContinuationBytesRejected() {
        assertFailsWith<JayoException> {
            newReader(Buffer().write("808080808001".decodeHex()))
                .readInt(0x1f, 0x1f)
        }.also { expected ->
            assertThat(expected.message).isEqualTo("HPACK integer overflow")
        }
    ...
    fun lowercaseHeaderNameBeforeEmit() {
        hpackWriter!!.writeHeaders(listOf(RealBinaryHeader("FoO", "BaR")))
    ...
    @Test
    fun emptyHeaderName() {
        hpackWriter!!.writeByteString("".encodeToByteString())
        assertBytes(0)
    ...
    fun emitsDynamicTableSizeUpdate() {
        ...
        hpackWriter!!.writeHeaders(listOf(RealBinaryHeader("bar", "foo")))
        assertBytes(
        ...
        hpackWriter!!.writeHeaders(listOf(RealBinaryHeader("far", "boo")))
    ...
    fun huffmanEncode() {
        ...
        val expected =
            Buffer()
                ...
                .writeByte(0x82.toByte()) // String literal is Huffman encoded (len = 2).
                .writeByte(0x94.toByte()) // 'foo' Huffman encoded.
                .writeByte(0xE7.toByte())
    ...
    private fun newReader(source: Buffer): Hpack.Reader = Hpack.Reader(source, 4096, 4096)

    private fun byteStream(vararg bytes: Int): Buffer = Buffer().write(intArrayToByteArray(bytes))

    private fun checkEntry(
        entry: RealBinaryHeader,
        name: String,
        value: String,
        size: Int,
    ) {
        assertThat(entry.name.decodeToString()).isEqualTo(name)
        assertThat(entry.value.decodeToString()).isEqualTo(value)
    ...
    private fun Buffer.contentEquals(other: Buffer) = this.snapshot() == other.snapshot()
}
