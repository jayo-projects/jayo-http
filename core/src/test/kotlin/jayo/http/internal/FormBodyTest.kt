...
class FormBodyTest {
    ...
    fun urlEncoding() {
        ...
        assertThat(body.contentByteSize()).isEqualTo(expected.length.toLong())
        val out = Buffer()
        body.writeTo(out)
        assertThat(out.readString()).isEqualTo(expected)
    }
    ...
    private fun formEncode(codePoint: Int): String {
        ...
        return buffer.readString(buffer.bytesAvailable() - 1) // Skip the "c" suffix.
    }
