...
class ConnectionCoalescingTest {
    ...
    fun setUp() {
        ...
        url = server.url("/robots.txt").toJayo()
    }
    ...
    fun coalescedConnectionDestroyedAfterAcquire() {
        ...
        val client1 =
            client
                ...
                .build()
        val request = ClientRequest.builder().url(sanUrl).get()
    ...
    /**
     * Skips coalescing when the hostname verifier is overridden since the intention of the hostname verification is a
     * black box.
     */
    @Test
    ...
    /**
     * Check we would use an existing connection to a later DNS result instead of connecting to the first DNS result for
     * the first time.
     */
    @Test
    ...
    private fun execute(url: HttpUrl) = client.newCall(ClientRequest.get(url)).execute()
