...
import jayo.http.internal.cache.RealCache.key
...
class CacheTest {
    ...
    private fun assertSubsequentResponseCached(
        ...
        val request = ClientRequest.builder()
            ...
            .get()
        val response = client.newCall(request).execute()
        assertThat(response.statusCode).isEqualTo(finalResponseCode)
    ...
    private fun testResponseCaching(transferKind: TransferKind) {
        ...
        assertThat(in1.readString("spiders".length.toLong())).isEqualTo("spiders")
        ...
        val in2 = response2.body.reader()
        assertThat(in2.readString("I love puppies but hate spiders".length.toLong()))
            .isEqualTo(
                "I love puppies but hate spiders",
            )
        assertThat(response2.statusCode).isEqualTo(200)
        assertThat(response2.statusMessage).isEqualTo("Fantastic")
    ...
    fun redirectToCachedResult() {
        ...
        val request1 = ClientRequest.builder().url(server.url("/foo").toJayo()).get()
        ...
        val request2 = ClientRequest.builder().url(server.url("/bar").toJayo()).get()
        ...
        val request3 = ClientRequest.builder().url(server.url("/baz").toJayo()).get()
    ...
    fun getAndQueryRedirectToCachedResultIndependently() {
        ...
        val requestGet1 = ClientRequest.builder()
        ...
        val requestQuery1 = ClientRequest.builder()
            .url(server.url("/foo").toJayo())
            .query(ClientRequestBody.EMPTY)
    ...
    private fun String.toOneShotRequestBody(): ClientRequestBody =
    ...
    fun secureResponseCachingAndRedirects() {
        ...
        val response1 = get(server.url("/").toJayo())
        assertThat(response1.body.string()).isEqualTo("ABC")
        assertThat(response1.handshake!!.cipherSuite).isNotNull()

        // Cached!
        val response2 = get(server.url("/").toJayo())
        assertThat(response2.body.string()).isEqualTo("ABC")
    ...
    /**
     * We've had bugs where caching and cross-protocol redirects yield class cast exceptions internal to the cache
     * because we incorrectly assumed that HttpsURLConnection was always HTTPS and HttpURLConnection was always HTTP; in
     * practice redirects mean that each can do either.
     *
    ...
    private fun temporaryRedirectCachedWithCachingHeader(
        ...
        val url = server.url("/")
        assertThat(get(url.toJayo()).body.string()).isEqualTo("a")
        assertThat(get(url.toJayo()).body.string()).isEqualTo("a")
    }
    ...
    private fun temporaryRedirectNotCachedWithoutCachingHeader(responseCode: Int) {
        ...
        val url = server.url("/")
        assertThat(get(url.toJayo()).body.string()).isEqualTo("a")
        assertThat(get(url.toJayo()).body.string()).isEqualTo("b")
    }
    ...
    private fun testServerPrematureDisconnect(transferKind: TransferKind) {
        ...
        val bodySource = get(server.url("/").toJayo()).body.reader()
        assertThat(bodySource.readLine()).isEqualTo("ABCDE")
        bodySource.use {
            assertFailsWith<JayoException> {
                bodySource.readString(21)
            }
        }
        assertThat(cache.writeAbortCount()).isEqualTo(1)
        assertThat(cache.writeSuccessCount()).isEqualTo(0)
        val response = get(server.url("/").toJayo())
    ...
    private fun testClientPrematureDisconnect(transferKind: TransferKind) {
        ...
        val response1 = get(server.url("/").toJayo())
        val source = response1.body.reader()
        assertThat(source.readString(5)).isEqualTo("ABCDE")
        source.close()
    ...
    fun defaultExpirationDateFullyCachedForMoreThan24Hours() {
        ...
        assertThat(get(server.url("/").toJayo()).body.string()).isEqualTo("A")
        val response = get(server.url("/").toJayo())
    ...
    fun maxAgeInThePastWithDateHeaderButNoLastModifiedHeader() {
        // Chrome interprets max-age relative to the local clock. Both our cache and Firefox both use the earlier of the
        // local and server's clock.
        assertNotCached(
            MockResponse
                .Builder()
                .addHeader("Date: " + formatDate(-120, TimeUnit.SECONDS))
    ...
    private fun requestBodyOrNull(requestMethod: String): ClientRequestBody? =
    ...
    private fun assertNonIdentityEncodingCached(response: MockResponse) {
        ...
        val url = server.url("/").toJayo()
        // At least three request/response pairs are required because after the first request is cached, a different
        // execution path might be taken. Thus, modifications to the cache applied during the second request might not
        // be visible until another request is performed.
        assertThat(get(url).body.string()).isEqualTo("ABCABCABC")
        assertThat(get(url).body.string()).isEqualTo("ABCABCABC")
        assertThat(get(url).body.string()).isEqualTo("ABCABCABC")
    }
    ...
    fun previouslyNotGzippedContentIsNotModifiedAndSpecifiesGzipEncoding() {
        ...
        assertThat(get(url).body.string()).isEqualTo("ABCABCABC")
        assertThat(get(url).body.string()).isEqualTo("ABCABCABC")
        assertThat(get(url).body.string()).isEqualTo("DEFDEFDEF")
    }
    ...
    fun requestOnlyIfCachedWithNoResponseCached() {
        ...
        val request = ClientRequest.builder()
            ...
            .get()
        val response = client.newCall(request).execute()
        assertThat(response.body.reader().exhausted()).isTrue()
    ...
    fun contentLocationDoesNotPopulateCache() {
        ...
        assertThat(get(server.url("/foo").toJayo()).body.string()).isEqualTo("A")
        assertThat(get(server.url("/bar").toJayo()).body.string()).isEqualTo("B")
    }
    ...
    fun connectionIsReturnedToPoolAfterConditionalSuccess() {
        ...
        assertThat(get(server.url("/a").toJayo()).body.string()).isEqualTo("A")
        assertThat(get(server.url("/a").toJayo()).body.string()).isEqualTo("A")
        assertThat(get(server.url("/b").toJayo()).body.string()).isEqualTo("B")
    ...
    fun statisticsConditionalCacheMiss() {
        ...
        assertThat(cache.networkCount()).isEqualTo(1)
        assertThat(cache.hitCount()).isEqualTo(0)
        assertThat(get(server.url("/").toJayo()).body.string()).isEqualTo("B")
        assertThat(get(server.url("/").toJayo()).body.string()).isEqualTo("C")
    ...
    fun conditionalHitUpdatesCache() {
        ...
        val t0 = Instant.now()
        val response1 = get(server.url("/a").toJayo())
        assertThat(response1.body.string()).isEqualTo("A")
        assertThat(response1.header("Allow")).isNull()
        assertThat(Duration.between(response1.receivedResponseAt, t0).toMillis()).isCloseTo(0, Offset.offset(250))

        // A conditional cache hit updates the cache.
        Thread.sleep(500) // Make sure t0 and t1 are distinct.
        val t1 = Instant.now()
        val response2 = get(server.url("/a").toJayo())
        assertThat(response2.statusCode).isEqualTo(HttpURLConnection.HTTP_OK)
        assertThat(response2.body.string()).isEqualTo("A")
        assertThat(response2.header("Allow")).isEqualTo("GET, HEAD")
        val updatedTimestamp = response2.receivedResponseAt
        assertThat(Duration.between(updatedTimestamp, t1).toMillis()).isCloseTo(0, Offset.offset(250))

        // A full cache hit reads the cache.
        Thread.sleep(10)
        val response3 = get(server.url("/a").toJayo())
        assertThat(response3.body.string()).isEqualTo("A")
        assertThat(response3.header("Allow")).isEqualTo("GET, HEAD")
        assertThat(response3.receivedResponseAt)
            // Truncate to milliseconds as dates are stored on filesystems with millisecond precision.
            .isEqualTo(updatedTimestamp.truncatedTo(ChronoUnit.MILLIS))
    ...
    fun testGoldenCacheHttpsResponse() {
      ...
      |
      """.trimMargin()
        val directoryPath = cache.directory
        writeFile(directoryPath, "$urlKey.0", entryMetadata)
        writeFile(directoryPath, "$urlKey.1", entryBody)
        writeFile(directoryPath, "journal", journalBody)
        cache.close()
        cache = Cache.create(directoryPath, Int.MAX_VALUE.toLong()) as RealCache
        client =
            client
                .newBuilder()
                .cache(cache)
    ...
    fun evictAll() {
        ...
        assertThat(client.cache!!.byteSize()).isEqualTo(0)
        assertThat(get(url).body.string()).isEqualTo("B")
    }
    ...
    @Disabled("OkHttp MockServer encodes headers by using UTF-8. We use ISO-8859-1")
    @Test
    fun combinedCacheHeadersCanBeNonAscii() {
        server.enqueue(
            MockResponse
                ...
                .addHeader("Last-Modified: " + formatDate(-1, TimeUnit.HOURS))
                .addHeader("Cache-Control: max-age=0")
                .addHeaderLenient("Square", "²")
                .addHeaderLenient("³", "Cube")
                .body("abcd")
                .build(),
        )
        server.enqueue(
            MockResponse
                .Builder()
                .addHeader("Transfer-Encoding: none")
                .addHeaderLenient("Micro", "µ")
                .addHeaderLenient("·", "Median")
                .code(HttpURLConnection.HTTP_NOT_MODIFIED)
                .build(),
        )
        val response1 = get(server.url("/").toJayo())
        assertThat(response1.header("Square")).isEqualTo("²")
        assertThat(response1.header("³")).isEqualTo("Cube")
        assertThat(response1.body.string()).isEqualTo("abcd")
        val response2 = get(server.url("/").toJayo())
        assertThat(response2.header("Square")).isEqualTo("²")
        assertThat(response2.header("³")).isEqualTo("Cube")
        assertThat(response2.header("Micro")).isEqualTo("µ")
        assertThat(response2.header("·")).isEqualTo("Median")
    ...
    fun getHasCorrectResponse() {
        val request = ClientRequest.get(server.url("/abc").toJayo())
    ...
    fun postWithOverrideResponse() {
        val url = server.url("/abc?token=123").toJayo()
    ...
    private fun testBasicCachingRules(request: ClientRequest): ClientResponse {
        val mockResponse =
            MockResponse
                .Builder()
                .addHeader("Last-Modified: " + formatDate(-1, TimeUnit.HOURS))
    ...
    /**
     * Shortens the body of `response` but not the corresponding headers. Only useful to test how clients respond to the
     * premature conclusion of the HTTP body.
     */
    ...
    private fun assertConditionallyCached(response: MockResponse): RecordedRequest {
        ...
        val valid = server.url("/valid").toJayo()
        val response1 = get(valid)
        assertThat(response1.body.string()).isEqualTo("A")
        assertThat(response1.statusCode).isEqualTo(HttpURLConnection.HTTP_OK)
        assertThat(response1.statusMessage).isEqualTo("A-OK")
        val response2 = get(valid)
        assertThat(response2.body.string()).isEqualTo("A")
        assertThat(response2.statusCode).isEqualTo(HttpURLConnection.HTTP_OK)
        assertThat(response2.statusMessage).isEqualTo("A-OK")
        val invalid = server.url("/invalid").toJayo()
        val response3 = get(invalid)
        assertThat(response3.body.string()).isEqualTo("B")
        assertThat(response3.statusCode).isEqualTo(HttpURLConnection.HTTP_OK)
        assertThat(response3.statusMessage).isEqualTo("B-OK")
        val response4 = get(invalid)
        assertThat(response4.body.string()).isEqualTo("C")
        assertThat(response4.statusCode).isEqualTo(HttpURLConnection.HTTP_OK)
        assertThat(response4.statusMessage).isEqualTo("C-OK")
    ...
    private operator fun get(url: HttpUrl): ClientResponse {
        val request = ClientRequest.builder()
            .url(url)
            .get()
        return client.newCall(request).execute()
    ...
    private fun writeFile(
        ...
        File.createIfNotExists(directory.resolve(file))
            .writer(StandardOpenOption.TRUNCATE_EXISTING)
            .buffered()
            .use {
                it.write(content)
            }
